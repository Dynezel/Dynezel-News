...
export default function Publicidades() {
  ...
  // Agregar bloques iniciales al cargar la página
  useEffect(() => {
    const initialBlocks = Math.floor(Math.random() * 3) + 5; // 5 a 7 bloques
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Siempre que estemos cerca del final, agregar 2-4 bloques
      if (scrollTop + clientHeight >= scrollHeight - 200) {
        const blocksToAdd = Math.floor(Math.random() * 3) + 2; // 2-4
        for (let i = 0; i < blocksToAdd; i++) addAdBlock();

        // Agregar Social Bar una sola vez al final
